...
language: en
modified: '2019-11-25T07:19:25.192809Z'
...
In this challenge  we will be projecting onto vector spaces (imagine a plane in 3D). The main goal will be to decompose each vector into two mutually orthogonal parts. This decomposition plays an important role in many applications, even where "orthogonality" has no geometric meaning. Moreover, we will see that orthogonality goes well together with eigenvectors and show that symmetric matrices are the best to work with (after diagonal matrices, of course).
...
5. Let `$W$` be a subspace of `$\mathbb R^n$` and `${\bf v}$` a vector in `$\mathbb R^n$`. Suppose that `${\bf w}$` and `${\bf w}'$` are orthogonal vectors with `${\bf w}\in W$` and  `${\bf v} = {\bf w} + {\bf w}'$`. Is it necessarily true that `${\bf w}'$` is in `$W^\perp$`? Either prove that it is true or find a counterexample. 

6. Let `$\{{\bf v}_1,\dots,{\bf v}_n\}$` be an orthogonal basis for `$\mathbb R^n$` and let `$W = {\rm span}({\bf v}_1,\dots,{\bf v}_k)$`. Is it necessarily true that `$W^\perp = {\rm span}({\bf v}_{k+1},\dots, {\bf v}_n)$`?  Either prove that it is true or find a counterexample. 
